...
const BeerMug: React.FC<{ className?: string }> = ({ className = '' }) => (
...
const LogoMark: React.FC<{ className?: string }> = ({ className = 'h-10 w-10' }) => (
...
const Brand: React.FC<{ dark?: boolean; className?: string }> = ({ dark = false, className = '' }) => (
  <div className={`flex items-center gap-3 ${className}`}>
    <LogoMark />
    <div className="leading-tight">
      <p className={`font-display text-lg font-semibold tracking-wide ${dark ? 'text-cream-100' : 'text-espresso-900'}`}>
        AHANDJO
      </p>
