 ...
 * - Vérification WhatsApp avant approbation
 * - Essai 7 jours
 */
...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onBackToHome, onBackToLogin, onRegistered }) => {
  ...
  const [verificationType, setVerificationType] = useState<'whatsapp' | 'email'>('whatsapp');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Crée et envoie le code de vérification WhatsApp
      const verifResult = await createVerificationCode(formData.whatsapp, 'whatsapp');

      if (!verifResult.success || !verifResult.verificationId) {
        setErrors({ whatsapp: verifResult.message || 'Impossible d\'envoyer le code de vérification' });
        setLoading(false);
        return;
      }
  ...
  const handleResendCode = async () => {
    const result = await createVerificationCode(formData.whatsapp, 'whatsapp');
    return result;
  };
        ...
        <div className="login-aside !hidden lg:!flex">
          ...
          <div className="login-aside-content">
            ...
            <p className="login-aside-sub">
              Rejoignez des centaines de gérants au Bénin et en Afrique de l'Ouest. 7 jours d'essai gratuit, sans carte bancaire.
            </p>
            ...
            <div className="space-y-4 mt-6">
              ...
              <div className="flex items-center gap-3 rounded-xl bg-white/10 p-3 backdrop-blur">
                ...
                <div>
                  <p className="text-sm font-bold text-cream-50">Vérification WhatsApp</p>
                  <p className="text-xs text-cream-100/70">Sécurité renforcée</p>
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-xl bg-white/10 p-3 backdrop-blur">
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-kente-green/20 text-emerald-300">
                  <BadgeCheck className="h-5 w-5" />
                </span>
                <div>
                  <p className="text-sm font-bold text-cream-50">Activation instantanée</p>
                  <p className="text-xs text-cream-100/70">Après paiement FEDAPAY</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="login-form-head">
            <div className="login-form-head-row">
              <div>
                ...
                <p className="login-form-sub">Créez votre compte en quelques secondes.</p>
              </div>
          ...
          <form className="login-form" onSubmit={handleSubmit} noValidate>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="login-field">
                  ...
                  <p className="text-xs text-espresso-400 mt-1">Code de vérification envoyé ici</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="login-field">
                ...
                <p className="text-xs text-espresso-400 mt-1">Laissez vide pour génération auto depuis le nom du bar</p>
              </div>
            ...
            <p className="text-xs text-espresso-400 text-center">
              En vous inscrivant, vous acceptez nos conditions d'utilisation. Vérification WhatsApp requise.
            </p>
          </form>
        </div>
      </div>
